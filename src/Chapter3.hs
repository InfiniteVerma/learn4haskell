...
fight (Knight kh ka kg) (Monster mh ma mg)
  ...
  | mh < 0 = kg + mg
  | mh == 0 && kh == 0 = 0
  | otherwise = fight (Knight (kh - mg) ka kg) (Monster (mh - ka) ma mg)
...
newtype House = House {residents :: [Human]} deriving (Show)
...
-- takes the Building part of City and checks if it's equal to Church
churchExists :: Building -> Bool
churchExists Church = True
churchExists _ = False

isChurchInCity :: City -> Bool
isChurchInCity city = churchExists (building city)

isCastleInCity :: City -> Bool
isCastleInCity city = not $ isChurchInCity city

wallNoUtil :: Maybe Castle -> Int
wallNoUtil (Just c) = cityWalls c
wallNoUtil _ = 0

wallNo :: City -> Int
wallNo city = wallNoUtil $ castle city

buildCastle :: City -> String -> City
buildCastle city name = city {castle = newCastle city name}
  where
    newCastle :: City -> String -> Maybe Castle
    newCastle city name = if isCastleInCity city then Just ((fromJust (castle city)) {castleName = name}) else Just (Castle {castleName = name, cityWalls = 0})

buildHouse :: City -> City
buildHouse city = city {houses = House (replicate peopleCount Human) : houses city}
  where
    peopleCount
      | isCastleInCity city = if isChurchInCity city then 4 else 3
      | otherwise = if isChurchInCity city then 2 else 1

residentsCount :: City -> Int
residentsCount city = sum $ map (length . residents) (houses city)

buildWalls :: City -> City
buildWalls city
  | not $ isCastleInCity city = city
  | residentsCount city < 10 = city
  | otherwise = city {castle = Just (newCastle city)}
  where
    newCastle :: City -> Castle
    newCastle city = (fromJust (castle city)) {cityWalls = wallNo city + 1}
...
calculatePlayerDefense armor dexterity = Defence (armorPts armor + dexPts dexterity)

calculatePlayerHit :: Damage -> Defence -> Health -> Health
calculatePlayerHit damage defense health = Health (healthPts health + defencePts defense - damagePts damage)
...
--   maybe-treasure ;)
newtype Power p = Power p deriving (Show)

newtype Dragon power = Dragon
  { dragonPower :: Power power
  }
  deriving (Show)
...
data Lair dPow = Lair
  { dragon :: Dragon dPow,
    chest :: Maybe (TreasureChest String)
  }
...
daysToParty day = if ind < iFriday then iFriday - ind else abs (iFriday - ind) + iFriday
  where
    ind = fromEnum day
...
newtype HealthT9 = HealthT9
  { healthT9Pts :: Int
  }
  deriving (Show)

newtype DefenceT9 = DefenceT9
  { defenceT9Pts :: Int
  }
  deriving (Show)

newtype AttackT9 = AttackT9
  { attackT9Pts :: Int
  }
  deriving (Show)

data KnightT9 = KnightT9
  { knightT9Health :: HealthT9,
    knightT9Defence :: DefenceT9,
    knightT9Attack :: AttackT9
  } deriving (Show)

data MonsterT9 = MonsterT9{
  monsterT9Health :: HealthT9,
  monsterT9Attack :: AttackT9
} deriving (Show)

-- list of actions that they can perform
